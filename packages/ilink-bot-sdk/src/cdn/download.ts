import { decryptAesEcb } from "./aes-ecb.js";
import { buildCdnDownloadUrl } from "./cdn-url.js";
import type { Logger } from "../logger.js";
import { defaultLogger } from "../logger.js";

/**
 * Download raw bytes from the CDN (no decryption).
 */
async function fetchCdnBytes(url: string, label: string, log: Logger): Promise<Buffer> {
  let res: Response;
  try {
    res = await fetch(url);
  } catch (err) {
    log.error(`${label}: fetch network error url=${url} err=${String(err)}`);
    throw err;
  }
  log.debug(`${label}: response status=${res.status} ok=${res.ok}`);
  if (!res.ok) {
    const body = await res.text().catch(() => "(unreadable)");
    const msg = `${label}: CDN download ${res.status} ${res.statusText} body=${body}`;
    log.error(msg);
    throw new Error(msg);
  }
  return Buffer.from(await res.arrayBuffer());
}

/**
 * Parse CDNMedia.aes_key into a raw 16-byte AES key.
 *
 * Two encodings are seen in the wild:
 *   - base64(raw 16 bytes)            — images (aes_key from media field)
 *   - base64(hex string of 16 bytes)  — file / voice / video
 */
function parseAesKey(aesKeyBase64: string, label: string, log: Logger): Buffer {
  const decoded = Buffer.from(aesKeyBase64, "base64");
  if (decoded.length === 16) {
    return decoded;
  }
  if (decoded.length === 32 && /^[0-9a-fA-F]{32}$/.test(decoded.toString("ascii"))) {
    return Buffer.from(decoded.toString("ascii"), "hex");
  }
  const msg = `${label}: aes_key must decode to 16 raw bytes or 32-char hex string, got ${decoded.length} bytes`;
  log.error(msg);
  throw new Error(msg);
}

/**
 * Download and AES-128-ECB decrypt a CDN media file. Returns plaintext Buffer.
 */
export async function downloadAndDecryptBuffer(
  encryptedQueryParam: string,
  aesKeyBase64: string,
  cdnBaseUrl: string,
  label: string,
  logger?: Logger,
): Promise<Buffer> {
  const log = logger ?? defaultLogger;
  const key = parseAesKey(aesKeyBase64, label, log);
  const url = buildCdnDownloadUrl(encryptedQueryParam, cdnBaseUrl);
  log.debug(`${label}: fetching url=${url}`);
  const encrypted = await fetchCdnBytes(url, label, log);
  log.debug(`${label}: downloaded ${encrypted.byteLength} bytes, decrypting`);
  const decrypted = decryptAesEcb(encrypted, key);
  log.debug(`${label}: decrypted ${decrypted.length} bytes`);
  return decrypted;
}

/**
 * Download plain (unencrypted) bytes from the CDN.
 */
export async function downloadPlainCdnBuffer(
  encryptedQueryParam: string,
  cdnBaseUrl: string,
  label: string,
  logger?: Logger,
): Promise<Buffer> {
  const log = logger ?? defaultLogger;
  const url = buildCdnDownloadUrl(encryptedQueryParam, cdnBaseUrl);
  log.debug(`${label}: fetching url=${url}`);
  return fetchCdnBytes(url, label, log);
}
